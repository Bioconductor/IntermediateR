...
other people interested in _R_ software development. It is suitable
for people who already know _R_, and are eager to develop a deeper
understanding of the language and 'best practices' when tackling
larger projects.
...
```
```{r echo=FALSE}
copyto("hi-1.R", file.path(pkg, "R", "hi.R"))
```
...
Load (if necessary) the newly installed package and check out the
functionality
...
Since functions are objects in _R_, implement the `how` argument so
that it takes a function rather than `character(1)`. What are the
strenghts and weaknesses of these approaches?
